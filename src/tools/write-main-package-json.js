...
module.exports = (packages, env) =>
  new Promise((resolve, reject) => {
    const requiredBump = R.last(
      R.flatten(packages.map(p => p[env.nsp].messages))
        .map(msg => msg.level)
        .sort()
    );
    ...
    if (requiredBump > -1) {
      ...
      const previouslyUnreleased = R.uniq(
        R.pathOr([], [env.nsp, 'previouslyUnreleased'], mainPkg).concat(
          packages
      ...
      mainPkg[env.nsp] = Object.assign({}, mainPkg[env.nsp], {
