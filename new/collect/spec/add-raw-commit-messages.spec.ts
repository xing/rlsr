...
const mockEnv: Env = {
  ...
  force: false,
  appRoot: "/",
  lastReleaseHash: "73087c7e82c715851fba548fd4d9bcd83234eaf9",
  currentHash: "c756e7d373ef983f3a2d3ae8c44ee1ba78bbad2e",
};

describe("add-raw-commit-messages", () => {
  let result: Env;
  beforeAll(async () => {
    const { addRawCommitMessages } = require("../add-raw-commit-messages");
    result = await addRawCommitMessages(mockEnv);
  });
  ...
  it("access simpleGit's log data since last release", () => {
    ...
    expect(mockSimpleGitLog).toHaveBeenCalledWith({
      from: `${mockEnv.lastReleaseHash}^1`,
  ...
  it("creates a picker to fetch Messages's relevant information", () => {
    ...
    expect(mockPick).toHaveBeenCalledWith(["hash", "date", "message", "body"]);
  });

  it("maps over simpleGit's log entries", () => {
    expect(mockPicker).toHaveBeenCalledTimes(mockSimpleGitLogResult.all.length);
  ...
  it("logs a yellow console message", () => {
    ...
    expect(mockYellow).toHaveBeenCalledWith(mockSimpleGitLogResult.all.length);
