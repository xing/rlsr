...
describe('updateVersionNumber()', () => {
  ...
  [['patch', 0, '1.2.4'], ['minor', 1, '1.3.0'], ['major', 2, '2.0.0']].map(t =>
    it(`bumps the current package with ${t[0]}`, () => {
      const p = R.clone(packages);
      p.one.rlsr.messages[0].level = t[1];

      updateVersionNumber({ nsp: 'rlsr' }, p)(p.one);
  ...
  [
    ...
    it(`accepts combined messages (${t[0]})`, () => {
      const p = R.clone(packages);
  ...
  [
    ...
    it(`marks related packages to bump (${t[0]})`, () => {
      const p = R.clone(packages);
      p.one.rlsr.messages[0].level = t[1];
      p.two.dependencies.one = t[2];

      updateVersionNumber({ nsp: 'rlsr', rlsrLatest: 'rlsr-latest' }, p)(p.one);
